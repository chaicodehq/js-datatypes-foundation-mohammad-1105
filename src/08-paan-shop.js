...
export function createPaanOrder(basePaan, customizations) {
  ...
  const newPaanOrder = Object.assign({}, basePaan, customizations);
  return newPaanOrder;
}
...
export function freezeMenu(menu) {
  ...
  const frozenMenu = Object.freeze(menu);
  return frozenMenu;
}
...
export function updatePrices(menu, increase) {
  ...
  const updatedEntries = menuEntries.map(([key, value]) => {
    return [key, value + increase];
  });

  const updatedMenu = Object.fromEntries(updatedEntries);
  return updatedMenu;
}
